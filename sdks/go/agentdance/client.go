...
func NewClientWithOptions(baseURL, apiKey string, options Options) (*Client, error) {
	...
	if u.Scheme != "https" || u.Host == "" {
		return nil, errors.New("AGENTDANCE base URL must be an absolute HTTPS URL")
	}
	if apiKey == "" {
		return nil, errors.New("AGENTDANCE API key is required")
	}
...
func (c *Client) do(ctx context.Context, method, path string, body []byte, contentType string, out any, retryable bool) error {
	...
	if err := json.Unmarshal(data, out); err != nil {
		return fmt.Errorf("decode AGENTDANCE response: %w", err)
	}
	return nil
}
...
func (c *Client) perform(ctx context.Context, method, path string, body []byte, contentType string, retryable bool) (*http.Response, []byte, error) {
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		req.Header.Set("User-Agent", "agentdance-go/1.0.0-rc.5")
	...
	if response == nil {
		return nil, nil, errors.New("AGENTDANCE request produced no response")
	}
	...
	if int64(len(data)) > c.maxResponseBytes {
		return nil, nil, errors.New("AGENTDANCE response exceeds MaxResponseBytes")
	}
