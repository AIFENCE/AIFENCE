...
class AifenceError(RuntimeError):
    def __init__(
        self,
        status_code: int,
        code: str,
...
class AifenceClient:
    def __init__(
        ...
        *,
        ...
        if not base_url.startswith("https://"):
            raise ValueError("AIFENCE base_url must use HTTPS")
        if not api_key and cert is None:
            raise ValueError("AIFENCE requires an API key or mTLS client certificate")
        if max_retries < 0:
    ...
    def __enter__(self) -> AifenceClient:
        return self
    ...
    def _request(self, method: str, path: str, *, retryable: bool = False, **kwargs: Any) -> httpx.Response:
        ...
        if response is None:
            raise RuntimeError("AIFENCE request did not produce a response")
...
class AsyncAifenceClient:
    ...
    async def __aenter__(self) -> AsyncAifenceClient:
        return self
