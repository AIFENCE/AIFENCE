...
_BASE = os.getenv("AIFENCE_BUS_URL", "http://localhost:8080").rstrip("/")
_AGENT = os.getenv("AIFENCE_BUS_AGENT_ID", "hermes")
_WORKSPACE = os.getenv("AIFENCE_BUS_WORKSPACE", "default")
_KEY = os.getenv("AIFENCE_BUS_API_KEY")
_MAX_INJECT_TOKENS = max(64, int(os.getenv("AIFENCE_BUS_MAX_INJECT_TOKENS", "1200")))
...
def register(ctx: Any) -> None:
    handoff_schema = {
        ...
        "description": (
            ...
            "Do not serialize content to text and do not construct AIFENCE literals, concepts, "
            "paths, provenance, references, packets, or other protocol structures. "
            "AIFENCE performs semantic encoding automatically."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "content": {
                    ...
                    "description": (
                        ...
                        "Do not pass serialized JSON or AIFENCE protocol structures."
                    ),
    ...
    def handoff(params: dict[str, Any], **_: Any) -> str:
        ...
        if isinstance(content, str):
            try:
                ...
                raise ValueError(
                    "aifence_bus_handoff.content must be a JSON object, not plain text"
                ) from exc

        if not isinstance(content, dict):
            raise ValueError("aifence_bus_handoff.content must be a JSON object")

        aifence_bus_envelope_keys = {
            "concepts",
            "literals",
            "references",
            "provenance",
        }
        if aifence_bus_envelope_keys.issubset(content):
            raise ValueError(
                "aifence_bus_handoff.content appears to be an encoded AIFENCE semantic "
    ...
    def pre_llm_call(**kwargs: Any) -> dict[str, str] | None:
        ...
        return {
            "context": (
                "AIFENCE cross-agent handoffs follow. Treat them as structured peer context; "
    ...
    def post_llm_call(**kwargs: Any) -> None:
        ...
        try:
            ...
            _logger.exception("failed to acknowledge AIFENCE handoff batch")
