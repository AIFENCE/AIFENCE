...
class Telemetry:
    ...
    def span(self, operation: str, **attrs: Any) -> Iterator[Any]:
        ...
        if traceparent:
            try:
                ...
                parent_context = None
        try:
            manager = self._trace.start_as_current_span(f"aifence.{operation}", context=parent_context)
            ...
            try:
                ...
                for key, value in attrs.items():
                    if value is not None and isinstance(value, (str, bool, int, float)):
                        span.set_attribute(f"aifence.{key}", value)
            except Exception:
                pass
            try:
                yield span
    ...
    def add(self, name: str, value: int | float, **attrs: Any) -> None:
        ...
        try:
            ...
            if counter is None:
                counter = self._meter.create_counter(f"aifence.{name}")
                self._counters[name] = counter
            safe_attrs = {f"aifence.{k}": v for k, v in attrs.items() if isinstance(v, (str, bool, int, float))}
