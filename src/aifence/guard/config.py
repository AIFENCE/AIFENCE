...
class Settings:
    ...
    artifact_s3_prefix: str = "aifence"
    ...
    operator_tenant_header: str = "X-Aifence-Tenant-ID"
    operator_identity_header: str = "X-Auth-Request-Email"
    operator_groups_header: str = "X-Auth-Request-Groups"
    operator_allowed_groups: tuple[str, ...] = ("aifence-operators",)
    ...
    def from_env(cls) -> Settings:
        ...
        # AIFENCE_GUARD_ names this loader reads. Legacy AIFENCE_GUARD_ still works.
        from ..core.env import adopt_legacy_prefix

        adopt_legacy_prefix("AGENTDANCE_", "AIFENCE_GUARD_")
        extra_file = os.getenv("AIFENCE_GUARD_EXTRA_JSON_FILE", "").strip()
        if extra_file:
            try:
                extra_raw = Path(extra_file).read_text()
            except OSError as exc:
                raise ValueError("AIFENCE_GUARD_EXTRA_JSON_FILE must reference a readable file") from exc
        else:
            extra_raw = os.getenv("AIFENCE_GUARD_EXTRA_JSON", "{}")
        try:
            extra = json.loads(extra_raw)
        except json.JSONDecodeError as exc:
            raise ValueError("AIFENCE_GUARD_EXTRA_JSON must be valid JSON") from exc
        if not isinstance(extra, dict):
            raise ValueError("AIFENCE_GUARD_EXTRA_JSON must contain a JSON object")
        settings = cls(
            environment=os.getenv("AIFENCE_GUARD_ENVIRONMENT", "development").lower(),
            runtime_role=os.getenv("AIFENCE_GUARD_RUNTIME_ROLE", "control-plane").lower(),
            bind_host=os.getenv("AIFENCE_GUARD_BIND_HOST", "0.0.0.0"),
            bind_port=_int("AIFENCE_GUARD_BIND_PORT", 8080),
            public_base_url=os.getenv("AIFENCE_GUARD_PUBLIC_BASE_URL", "http://localhost:8080"),
            source_code_url=os.getenv("AIFENCE_GUARD_SOURCE_CODE_URL", "https://github.com/AIFENCE/AIFENCE"),
            commercial_license_url=os.getenv("AIFENCE_GUARD_COMMERCIAL_LICENSE_URL", "https://github.com/AIFENCE/AIFENCE/blob/main/COMMERCIAL-LICENSE.md"),
            database_url=_secret_value(
                "AIFENCE_GUARD_DATABASE_URL", "AIFENCE_GUARD_DATABASE_URL_FILE",
                "sqlite+pysqlite:///./aifence.db"
            ),
            db_pool_size=_int("AIFENCE_GUARD_DB_POOL_SIZE", 20),
            db_max_overflow=_int("AIFENCE_GUARD_DB_MAX_OVERFLOW", 20),
            auto_create_schema=_bool("AIFENCE_GUARD_AUTO_CREATE_SCHEMA", True),
            signing_private_key_file=os.getenv(
                "AIFENCE_GUARD_SIGNING_PRIVATE_KEY_FILE", "./secrets/signing-private.pem"
            ),
            signing_public_key_file=os.getenv(
                "AIFENCE_GUARD_SIGNING_PUBLIC_KEY_FILE", "./secrets/signing-public.pem"
            ),
            signing_key_id=os.getenv("AIFENCE_GUARD_SIGNING_KEY_ID", "aifence-signing-v1"),
            signing_backend=os.getenv("AIFENCE_GUARD_SIGNING_BACKEND", "local").lower(),
            signing_vault_address=os.getenv("AIFENCE_GUARD_SIGNING_VAULT_ADDRESS", ""),
            signing_vault_token=_secret_value("AIFENCE_GUARD_SIGNING_VAULT_TOKEN", "AIFENCE_GUARD_SIGNING_VAULT_TOKEN_FILE"),
            signing_vault_mount=os.getenv("AIFENCE_GUARD_SIGNING_VAULT_MOUNT", "transit"),
            signing_vault_key_name=os.getenv("AIFENCE_GUARD_SIGNING_VAULT_KEY_NAME", "aifence-signing"),
            master_key_b64=_secret_value(
                "AIFENCE_GUARD_MASTER_KEY_B64", "AIFENCE_GUARD_MASTER_KEY_B64_FILE"
            ),
            master_key_id=os.getenv("AIFENCE_GUARD_MASTER_KEY_ID", "master-v1"),
            master_keyring_json=_secret_value(
                "AIFENCE_GUARD_MASTER_KEYRING_JSON", "AIFENCE_GUARD_MASTER_KEYRING_JSON_FILE"
            ),
            api_key_pepper=_secret_value(
                "AIFENCE_GUARD_API_KEY_PEPPER", "AIFENCE_GUARD_API_KEY_PEPPER_FILE"
            ),
            api_key_pepper_id=os.getenv("AIFENCE_GUARD_API_KEY_PEPPER_ID", "pepper-v1"),
            api_key_pepperring_json=_secret_value(
                "AIFENCE_GUARD_API_KEY_PEPPERRING_JSON",
                "AIFENCE_GUARD_API_KEY_PEPPERRING_JSON_FILE",
            ),
            tls_cert_file=os.getenv("AIFENCE_GUARD_TLS_CERT_FILE", ""),
            tls_key_file=os.getenv("AIFENCE_GUARD_TLS_KEY_FILE", ""),
            tls_client_ca_file=os.getenv("AIFENCE_GUARD_TLS_CLIENT_CA_FILE", ""),
            require_mtls=_bool("AIFENCE_GUARD_REQUIRE_MTLS", False),
            trusted_proxy_cidrs=_csv("AIFENCE_GUARD_TRUSTED_PROXY_CIDRS"),
            internal_cidrs=_csv("AIFENCE_GUARD_INTERNAL_CIDRS"),
            allowed_origins=_csv("AIFENCE_GUARD_ALLOWED_ORIGINS"),
            docs_enabled=_bool("AIFENCE_GUARD_DOCS_ENABLED", True),
            max_request_bytes=_int("AIFENCE_GUARD_MAX_REQUEST_BYTES", 2 * 1024 * 1024),
            max_artifact_bytes=_int("AIFENCE_GUARD_MAX_ARTIFACT_BYTES", 10 * 1024 * 1024),
            max_broker_response_bytes=_int("AIFENCE_GUARD_MAX_BROKER_RESPONSE_BYTES", 4 * 1024 * 1024),
            request_timeout_seconds=_int("AIFENCE_GUARD_REQUEST_TIMEOUT_SECONDS", 30),
            proxy_timeout_seconds=_int("AIFENCE_GUARD_PROXY_TIMEOUT_SECONDS", 60),
            dns_resolution_timeout_seconds=_int("AIFENCE_GUARD_DNS_RESOLUTION_TIMEOUT_SECONDS", 3),
            egress_proxy_url=os.getenv("AIFENCE_GUARD_EGRESS_PROXY_URL", ""),
            clamav_host=os.getenv("AIFENCE_GUARD_CLAMAV_HOST", "127.0.0.1"),
            clamav_port=_int("AIFENCE_GUARD_CLAMAV_PORT", 3310),
            clamav_required=_bool("AIFENCE_GUARD_CLAMAV_REQUIRED", False),
            clamav_timeout_seconds=_int("AIFENCE_GUARD_CLAMAV_TIMEOUT_SECONDS", 20),
            rate_limit_per_minute=_int("AIFENCE_GUARD_RATE_LIMIT_PER_MINUTE", 600),
            audit_retention_days=_int("AIFENCE_GUARD_AUDIT_RETENTION_DAYS", 2555),
            artifact_retention_days=_int("AIFENCE_GUARD_ARTIFACT_RETENTION_DAYS", 30),
            artifact_store_path=os.getenv("AIFENCE_GUARD_ARTIFACT_STORE_PATH", "./data/artifacts"),
            artifact_store_backend=os.getenv("AIFENCE_GUARD_ARTIFACT_STORE_BACKEND", "file").lower(),
            artifact_s3_endpoint=os.getenv("AIFENCE_GUARD_ARTIFACT_S3_ENDPOINT", ""),
            artifact_s3_bucket=os.getenv("AIFENCE_GUARD_ARTIFACT_S3_BUCKET", ""),
            artifact_s3_region=os.getenv("AIFENCE_GUARD_ARTIFACT_S3_REGION", "us-east-1"),
            artifact_s3_access_key=_secret_value("AIFENCE_GUARD_ARTIFACT_S3_ACCESS_KEY", "AIFENCE_GUARD_ARTIFACT_S3_ACCESS_KEY_FILE"),
            artifact_s3_secret_key=_secret_value("AIFENCE_GUARD_ARTIFACT_S3_SECRET_KEY", "AIFENCE_GUARD_ARTIFACT_S3_SECRET_KEY_FILE"),
            artifact_s3_prefix=os.getenv("AIFENCE_GUARD_ARTIFACT_S3_PREFIX", "aifence"),
            artifact_s3_kms_key_id=os.getenv("AIFENCE_GUARD_ARTIFACT_S3_KMS_KEY_ID", ""),
            artifact_s3_object_lock_days=_int("AIFENCE_GUARD_ARTIFACT_S3_OBJECT_LOCK_DAYS", 0),
            artifact_s3_delete_enabled=_bool("AIFENCE_GUARD_ARTIFACT_S3_DELETE_ENABLED", False),
            audit_checkpoint_interval=_int("AIFENCE_GUARD_AUDIT_CHECKPOINT_INTERVAL", 1000),
            audit_anchor_backend=os.getenv("AIFENCE_GUARD_AUDIT_ANCHOR_BACKEND", "file").lower(),
            audit_anchor_directory=os.getenv("AIFENCE_GUARD_AUDIT_ANCHOR_DIRECTORY", "./data/audit-anchors"),
            audit_anchor_webhook_url=os.getenv("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_URL", ""),
            audit_anchor_webhook_token=_secret_value("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_TOKEN", "AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_TOKEN_FILE"),
            audit_anchor_webhook_verify_url=os.getenv("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_VERIFY_URL", ""),
            audit_anchor_webhook_public_key_file=os.getenv("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_PUBLIC_KEY_FILE", ""),
            audit_anchor_webhook_key_ids=_csv("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_KEY_IDS"),
            max_page_size=_int("AIFENCE_GUARD_MAX_PAGE_SIZE", 500),
            execution_lease_seconds=_int("AIFENCE_GUARD_EXECUTION_LEASE_SECONDS", 120),
            dispatch_mode=os.getenv("AIFENCE_GUARD_DISPATCH_MODE", "inline").lower(),
            dispatch_wait_seconds=float(os.getenv("AIFENCE_GUARD_DISPATCH_WAIT_SECONDS", "3")),
            worker_batch_size=_int("AIFENCE_GUARD_WORKER_BATCH_SIZE", 20),
            worker_concurrency=_int("AIFENCE_GUARD_WORKER_CONCURRENCY", 8),
            worker_lease_renewal_seconds=_int("AIFENCE_GUARD_WORKER_LEASE_RENEWAL_SECONDS", 30),
            worker_poll_milliseconds=_int("AIFENCE_GUARD_WORKER_POLL_MILLISECONDS", 500),
            worker_max_attempts=_int("AIFENCE_GUARD_WORKER_MAX_ATTEMPTS", 5),
            worker_retry_base_seconds=_int("AIFENCE_GUARD_WORKER_RETRY_BASE_SECONDS", 2),
            workload_auth_enabled=_bool("AIFENCE_GUARD_WORKLOAD_AUTH_ENABLED", False),
            workload_trust_domains=_csv("AIFENCE_GUARD_WORKLOAD_TRUST_DOMAINS"),
            workload_identity_header=os.getenv("AIFENCE_GUARD_WORKLOAD_IDENTITY_HEADER", "X-Forwarded-Client-Cert"),
            workload_identity_direct_header=os.getenv("AIFENCE_GUARD_WORKLOAD_IDENTITY_DIRECT_HEADER", "X-SPIFFE-ID"),
            policy_rollout_secret=_secret_value("AIFENCE_GUARD_POLICY_ROLLOUT_SECRET", "AIFENCE_GUARD_POLICY_ROLLOUT_SECRET_FILE"),
            kms_backend=os.getenv("AIFENCE_GUARD_KMS_BACKEND", "local").lower(),
            kms_key_id=os.getenv("AIFENCE_GUARD_KMS_KEY_ID", ""),
            kms_decryption_key_ids=_csv("AIFENCE_GUARD_KMS_DECRYPTION_KEY_IDS"),
            tenant_key_backend=os.getenv("AIFENCE_GUARD_TENANT_KEY_BACKEND", os.getenv("AIFENCE_GUARD_KMS_BACKEND", "local")).lower(),
            tenant_kms_key_template=os.getenv("AIFENCE_GUARD_TENANT_KMS_KEY_TEMPLATE", ""),
            tenant_key_destroy_enabled=_bool("AIFENCE_GUARD_TENANT_KEY_DESTROY_ENABLED", False),
            lifecycle_lease_seconds=_int("AIFENCE_GUARD_LIFECYCLE_LEASE_SECONDS", 300),
            lifecycle_poll_milliseconds=_int("AIFENCE_GUARD_LIFECYCLE_POLL_MILLISECONDS", 1000),
            lifecycle_batch_size=_int("AIFENCE_GUARD_LIFECYCLE_BATCH_SIZE", 10),
            lifecycle_max_attempts=_int("AIFENCE_GUARD_LIFECYCLE_MAX_ATTEMPTS", 5),
            anchor_lease_seconds=_int("AIFENCE_GUARD_ANCHOR_LEASE_SECONDS", 120),
            anchor_poll_milliseconds=_int("AIFENCE_GUARD_ANCHOR_POLL_MILLISECONDS", 1000),
            anchor_batch_size=_int("AIFENCE_GUARD_ANCHOR_BATCH_SIZE", 20),
            anchor_max_attempts=_int("AIFENCE_GUARD_ANCHOR_MAX_ATTEMPTS", 8),
            audit_anchor_required_quorum=_int("AIFENCE_GUARD_AUDIT_ANCHOR_REQUIRED_QUORUM", 1),
            audit_anchor_destinations=_csv("AIFENCE_GUARD_AUDIT_ANCHOR_DESTINATIONS"),
            vault_address=os.getenv("AIFENCE_GUARD_VAULT_ADDRESS", ""),
            vault_token=_secret_value("AIFENCE_GUARD_VAULT_TOKEN", "AIFENCE_GUARD_VAULT_TOKEN_FILE"),
            vault_transit_mount=os.getenv("AIFENCE_GUARD_VAULT_TRANSIT_MOUNT", "transit"),
            operator_console_enabled=_bool("AIFENCE_GUARD_OPERATOR_CONSOLE_ENABLED", False),
            operator_tenant_header=os.getenv("AIFENCE_GUARD_OPERATOR_TENANT_HEADER", "X-Aifence-Tenant-ID"),
            operator_identity_header=os.getenv("AIFENCE_GUARD_OPERATOR_IDENTITY_HEADER", "X-Auth-Request-Email"),
            operator_groups_header=os.getenv("AIFENCE_GUARD_OPERATOR_GROUPS_HEADER", "X-Auth-Request-Groups"),
            operator_allowed_groups=_csv("AIFENCE_GUARD_OPERATOR_ALLOWED_GROUPS", ("aifence-operators",)),
            otel_service_name=os.getenv("AIFENCE_GUARD_OTEL_SERVICE_NAME", "aifence"),
            otel_exporter_otlp_endpoint=os.getenv(
                "AIFENCE_GUARD_OTEL_EXPORTER_OTLP_ENDPOINT", ""
            ),
            log_level=os.getenv("AIFENCE_GUARD_LOG_LEVEL", "INFO").upper(),
            policy_file=os.getenv("AIFENCE_GUARD_POLICY_FILE", ""),
            provider_allowed_hosts=_csv("AIFENCE_GUARD_PROVIDER_ALLOWED_HOSTS"),
            tool_allowed_hosts=_csv("AIFENCE_GUARD_TOOL_ALLOWED_HOSTS"),
            shutdown_grace_seconds=_int("AIFENCE_GUARD_SHUTDOWN_GRACE_SECONDS", 30),
            extra=extra,
        )
        settings.validate()
        return settings
    ...
    def encryption_keyring(self) -> tuple[str, dict[str, bytes]]:
        ...
        if not active_id or len(active_id.encode("utf-8")) > 255:
            raise ValueError("AIFENCE_GUARD_MASTER_KEY_ID must contain 1 to 255 UTF-8 bytes")
        keys: dict[str, bytes] = {}
        if self.master_keyring_json:
            try:
                document = json.loads(self.master_keyring_json)
            except json.JSONDecodeError as exc:
                raise ValueError("AIFENCE_GUARD_MASTER_KEYRING_JSON must be valid JSON") from exc
            if not isinstance(document, dict) or not document:
                raise ValueError("AIFENCE_GUARD_MASTER_KEYRING_JSON must be a non-empty object")
        ...
        if self.master_key_b64:
            decoded = self._decode_master_key(self.master_key_b64, "AIFENCE_GUARD_MASTER_KEY_B64")
        ...
        if not keys:
            if self.environment == "production":
                raise ValueError("an AIFENCE master encryption key is required in production")
            keys[active_id] = bytes.fromhex("1f" * 32)
        if active_id not in keys:
            raise ValueError("AIFENCE_GUARD_MASTER_KEY_ID is not present in the configured keyring")
    ...
    def peppers(self) -> tuple[str, dict[str, bytes]]:
        ...
        if not active_id or len(active_id.encode("utf-8")) > 255:
            raise ValueError("AIFENCE_GUARD_API_KEY_PEPPER_ID must contain 1 to 255 UTF-8 bytes")
        values: dict[str, bytes] = {}
        if self.api_key_pepperring_json:
            try:
                document = json.loads(self.api_key_pepperring_json)
            except json.JSONDecodeError as exc:
                raise ValueError("AIFENCE_GUARD_API_KEY_PEPPERRING_JSON must be valid JSON") from exc
            if not isinstance(document, dict) or not document:
                raise ValueError("AIFENCE_GUARD_API_KEY_PEPPERRING_JSON must be a non-empty object")
        ...
        if self.api_key_pepper:
            ...
            if len(encoded) < 32:
                raise ValueError("AIFENCE_GUARD_API_KEY_PEPPER must contain at least 32 bytes")
        ...
        if not values:
            if self.environment == "production":
                raise ValueError("an AIFENCE API key pepper is required in production")
            values[active_id] = b"development-only-pepper-change-before-production"
        if active_id not in values:
            raise ValueError("AIFENCE_GUARD_API_KEY_PEPPER_ID is not present in the configured pepperring")
    ...
    def validate(self) -> None:
        if self.environment not in {"development", "test", "staging", "production"}:
            raise ValueError("AIFENCE_GUARD_ENVIRONMENT has an unsupported value")
        if self.runtime_role not in {"control-plane", "dispatcher", "lifecycle", "anchor", "migration", "maintenance"}:
            raise ValueError("AIFENCE_GUARD_RUNTIME_ROLE has an unsupported value")
        if self.bind_port < 1 or self.bind_port > 65535:
            raise ValueError("AIFENCE_GUARD_BIND_PORT must be between 1 and 65535")
        for label, value in (("AIFENCE_GUARD_SOURCE_CODE_URL", self.source_code_url), ("AIFENCE_GUARD_COMMERCIAL_LICENSE_URL", self.commercial_license_url)):
            parsed = urlparse(value)
            if parsed.scheme not in {"http", "https"} or not parsed.netloc:
                raise ValueError(f"{label} must be an absolute HTTP(S) URL")
        if self.max_request_bytes < 1024:
            raise ValueError("AIFENCE_GUARD_MAX_REQUEST_BYTES is too small")
        if self.max_artifact_bytes < 1024:
            raise ValueError("AIFENCE_GUARD_MAX_ARTIFACT_BYTES is too small")
        if self.max_broker_response_bytes < 1024:
            raise ValueError("AIFENCE_GUARD_MAX_BROKER_RESPONSE_BYTES is too small")
        if self.audit_checkpoint_interval < 1:
            raise ValueError("AIFENCE_GUARD_AUDIT_CHECKPOINT_INTERVAL must be positive")
        if not 1 <= self.max_page_size <= 5000:
            raise ValueError("AIFENCE_GUARD_MAX_PAGE_SIZE is outside the supported range")
        if not 30 <= self.execution_lease_seconds <= 3600:
            raise ValueError("AIFENCE_GUARD_EXECUTION_LEASE_SECONDS must be between 30 and 3600")
        ...
        if not 1 <= self.rate_limit_per_minute <= 1_000_000:
            raise ValueError("AIFENCE_GUARD_RATE_LIMIT_PER_MINUTE is outside the supported range")
        if not 1 <= self.proxy_timeout_seconds <= 600:
            raise ValueError("AIFENCE_GUARD_PROXY_TIMEOUT_SECONDS is outside the supported range")
        if not 1 <= self.dns_resolution_timeout_seconds <= 30:
            raise ValueError("AIFENCE_GUARD_DNS_RESOLUTION_TIMEOUT_SECONDS is outside the supported range")
        ...
        if self.dispatch_mode not in {"inline", "async", "hybrid"}:
            raise ValueError("AIFENCE_GUARD_DISPATCH_MODE must be inline, async, or hybrid")
        ...
        if not 1 <= self.worker_concurrency <= 256:
            raise ValueError("AIFENCE_GUARD_WORKER_CONCURRENCY must be between 1 and 256")
        ...
        if self.kms_backend not in {"local", "aws", "gcp", "azure", "vault"}:
            raise ValueError("AIFENCE_GUARD_KMS_BACKEND is unsupported")
        if self.tenant_key_backend not in {"local", "aws", "gcp", "azure", "vault"}:
            raise ValueError("AIFENCE_GUARD_TENANT_KEY_BACKEND is unsupported")
        ...
        if self.signing_backend not in {"local", "vault"}:
            raise ValueError("AIFENCE_GUARD_SIGNING_BACKEND is unsupported")
        if self.artifact_store_backend not in {"file", "s3"}:
            raise ValueError("AIFENCE_GUARD_ARTIFACT_STORE_BACKEND is unsupported")
        if self.audit_anchor_backend not in {"file", "webhook"}:
            raise ValueError("AIFENCE_GUARD_AUDIT_ANCHOR_BACKEND is unsupported")
        for label, cidrs in (
            ("AIFENCE_GUARD_TRUSTED_PROXY_CIDRS", self.trusted_proxy_cidrs),
            ("AIFENCE_GUARD_INTERNAL_CIDRS", self.internal_cidrs),
        ):
            for cidr in cidrs:
                try:
        ...
        if self.environment == "production" and self.runtime_role in {"lifecycle", "anchor"}:
            ...
            if not self.internal_cidrs:
                failures.append("AIFENCE_GUARD_INTERNAL_CIDRS is required")
        ...
        if self.environment == "production":
            ...
            if self.auto_create_schema:
                failures.append("AIFENCE_GUARD_AUTO_CREATE_SCHEMA must be false")
            ...
            if (
                ...
                failures.append(
                    "AIFENCE_GUARD_PUBLIC_BASE_URL must be a canonical HTTPS origin without credentials, query, or fragment"
                )
            for label, value in (
                ("AIFENCE_GUARD_TLS_CERT_FILE", self.tls_cert_file),
                ("AIFENCE_GUARD_TLS_KEY_FILE", self.tls_key_file),
            ):
            ...
            if not self.require_mtls:
                failures.append("AIFENCE_GUARD_REQUIRE_MTLS must be true in production")
            if not self.tls_client_ca_file or not Path(self.tls_client_ca_file).is_file():
                failures.append("production mTLS requires AIFENCE_GUARD_TLS_CLIENT_CA_FILE")
            if not Path(self.artifact_store_path).is_absolute():
                failures.append("AIFENCE_GUARD_ARTIFACT_STORE_PATH must be absolute")
            proxy_url = urlparse(self.egress_proxy_url)
            if (
                ...
                failures.append(
                    "AIFENCE_GUARD_EGRESS_PROXY_URL must be an HTTP(S) proxy URL without embedded credentials, query, or fragment"
                )
            if not self.clamav_required:
                failures.append("AIFENCE_GUARD_CLAMAV_REQUIRED must be true")
            if "*" in self.allowed_origins:
                failures.append("wildcard CORS is prohibited")
            if self.docs_enabled:
                failures.append("interactive API docs must be disabled")
            for label, patterns in (
                ("AIFENCE_GUARD_PROVIDER_ALLOWED_HOSTS", self.provider_allowed_hosts),
                ("AIFENCE_GUARD_TOOL_ALLOWED_HOSTS", self.tool_allowed_hosts),
            ):
            ...
            if self.policy_file and not Path(self.policy_file).is_file():
                failures.append("AIFENCE_GUARD_POLICY_FILE must reference a readable file")
            ...
            if not self.trusted_proxy_cidrs:
                failures.append("production workload authentication requires AIFENCE_GUARD_TRUSTED_PROXY_CIDRS")
            if self.kms_backend == "local" or not self.kms_key_id:
            ...
            if not self.policy_rollout_secret or len(self.policy_rollout_secret.encode("utf-8")) < 32:
                failures.append("AIFENCE_GUARD_POLICY_ROLLOUT_SECRET must contain at least 32 bytes")
            if self.audit_anchor_backend == "file":
                ...
                if (anchor_url.scheme.lower() != "https" or not anchor_url.hostname
                    ...
                    failures.append("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_URL must be a canonical HTTPS URL")
                if len(self.audit_anchor_webhook_token.encode("utf-8")) < 32:
                    failures.append("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_TOKEN must contain at least 32 bytes")
                ...
                if (verify_url.scheme.lower() != "https" or not verify_url.hostname
                    ...
                    failures.append("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_VERIFY_URL must be canonical HTTPS")
                if not self.audit_anchor_webhook_public_key_file or not Path(self.audit_anchor_webhook_public_key_file).is_file():
                    failures.append("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_PUBLIC_KEY_FILE must reference a readable key")
                if not self.audit_anchor_webhook_key_ids:
                    failures.append("AIFENCE_GUARD_AUDIT_ANCHOR_WEBHOOK_KEY_IDS must allow at least one receipt key")
            if self.artifact_store_backend != "s3":
                ...
                if (s3_url.scheme.lower() != "https" or not s3_url.hostname
                    ...
                    failures.append("AIFENCE_GUARD_ARTIFACT_S3_ENDPOINT must be a canonical HTTPS origin")
                if not self.artifact_s3_bucket:
                    failures.append("S3 artifact storage requires AIFENCE_GUARD_ARTIFACT_S3_BUCKET")
                ...
                if not self.artifact_s3_kms_key_id:
                    failures.append("production S3 artifact storage requires AIFENCE_GUARD_ARTIFACT_S3_KMS_KEY_ID")
            ...
            if self.kms_backend == "vault":
                ...
                if (vault_url.scheme.lower() != "https" or not vault_url.hostname
                    ...
                    failures.append("AIFENCE_GUARD_VAULT_ADDRESS must be a canonical HTTPS origin")
                if not self.vault_token:
                    failures.append("Vault KMS requires AIFENCE_GUARD_VAULT_TOKEN")
            try:
