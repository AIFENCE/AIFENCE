...
def render_typescript() -> str:
    ...
    lines = ["export type AifenceWireV2 = {", "  v: 2;", "  c: string;"]
    for key in keys:
        if key in {"v", "c"}:
            continue
        lines.append(f"  {json.dumps(key)}?: unknown;")
    lines.extend(["};", "", f"export const AIFENCE_PROTOCOL = {json.dumps(AIFENCE_PROTOCOL)} as const;", f"export const AIFENCE_WIRE_VERSION = {AIFENCE_WIRE_VERSION} as const;", ""])
    return "\n".join(lines)
...
def render_go() -> str:
    schema = WirePacketV2.model_json_schema(by_alias=True)
    keys = sorted(schema["properties"])
    lines = ["package aifencewire", "", "const Protocol = \"aifence/0.2\"", "const WireVersion = 2", "", "type Packet map[string]any", "", "var WireKeys = map[string]struct{}{",]
...
def render_field_table() -> str:
    ...
    lines = ["# Generated wire fields", "", "This file is generated from `aifence.bus.protocol_spec.WirePacketV2`.", "", "| Field | Required |", "|---|---|" ]
...
def main() -> None:
    ...
    values = artifacts()
    manifest = {"protocol": AIFENCE_PROTOCOL, "wire": AIFENCE_WIRE_VERSION, "artifacts": digest_map(values)}
