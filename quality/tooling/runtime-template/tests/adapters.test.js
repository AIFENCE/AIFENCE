...
test('project installer dry run touches only project paths',()=>{const p=fs.mkdtempSync(path.join(os.tmpdir(),'aifence-install-'));const x=install('all',{project:p,dryRun:true});assert.equal(x.ok,true);assert.ok(x.changed.every(f=>path.resolve(f).startsWith(path.resolve(p))));assert.equal(fs.readdirSync(p).length,0)});
test('project installer merges without clobbering existing config',()=>{const p=fs.mkdtempSync(path.join(os.tmpdir(),'aifence-install-'));fs.mkdirSync(path.join(p,'.vscode'));fs.writeFileSync(path.join(p,'.vscode','mcp.json'),JSON.stringify({servers:{existing:{type:'http',url:'https://example.invalid'}}}));install('vscode',{project:p});const j=JSON.parse(fs.readFileSync(path.join(p,'.vscode','mcp.json')));assert.ok(j.servers.existing);assert.ok(j.servers.aifence);assert.ok(fs.existsSync(path.join(p,'.vscode','mcp.json.aifence-backup')))});

test('install all mutates only project-scoped adapter namespaces and preserves existing JSON', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'aifence-install-all-'));
  try {
    ...
    assert.ok(fs.existsSync(path.join(tmp, '.agents', 'skills', 'aifence', 'SKILL.md')));
    assert.ok(fs.existsSync(path.join(tmp, '.claude', 'skills', 'aifence', 'SKILL.md')));
    ...
    assert.ok(cursor.mcpServers.aifence.command);
    assert.ok(fs.existsSync(path.join(tmp, '.cursor', 'mcp.json.aifence-backup')));
    const backup = JSON.parse(fs.readFileSync(path.join(tmp, '.cursor', 'mcp.json.aifence-backup'), 'utf8'));
    assert.equal(backup.mcpServers.existing.command, 'keep-me');
    assert.equal(backup.mcpServers.aifence, undefined);
  } finally {
