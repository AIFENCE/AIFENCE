...
function backup(p,dry){if(fs.existsSync(p)&&!dry){const b=`${p}.aifence-backup`; if(!fs.existsSync(b))fs.copyFileSync(p,b);}}
...
function skill(target,dry){const d=path.join(target,'.agents','skills','aifence');mkdir(path.dirname(d),dry);cp(SKILL_ROOT,d,dry);return d}
...
export function install(targetKind,{project=process.cwd(),dryRun=false}={}){const p=path.resolve(project);const changed=[]; const ensureSkill=()=>{const d=skill(p,dryRun);changed.push(d)};
 ...
 if(['claude','all'].includes(targetKind)){const d=path.join(p,'.claude','skills','aifence');cp(SKILL_ROOT,d,dryRun);changed.push(d);const f=path.join(p,'.mcp.json');mergeJson(f,{mcpServers:{aifence:mcpCmd()}},dryRun);changed.push(f)}
 if(['gemini','all'].includes(targetKind)){const f=path.join(p,'.gemini','settings.json');mergeJson(f,{mcpServers:{aifence:mcpCmd()}},dryRun);changed.push(f)}
 if(['vscode','all'].includes(targetKind)){const f=path.join(p,'.vscode','mcp.json');mergeJson(f,{servers:{aifence:{type:'stdio',...mcpCmd()}}},dryRun);changed.push(f)}
 if(['cursor','all'].includes(targetKind)){const f=path.join(p,'.cursor','mcp.json');mergeJson(f,{mcpServers:{aifence:mcpCmd()}},dryRun);changed.push(f)}
