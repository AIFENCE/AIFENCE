...
if(!['127.0.0.1','localhost','::1'].includes(host) && !process.env.BIZIQ_ALLOW_REMOTE_HTTP){console.error('Refusing non-loopback bind without BIZIQ_ALLOW_REMOTE_HTTP=1. Put remote deployments behind authenticated TLS/reverse-proxy policy.');process.exit(2)}
const handler=createMcpHandler(createBizIQServer); const nodeHandler=toNodeHandler(handler);
...
server.listen(port,host,()=>console.error(`BizIQ Runtime MCP HTTP http://${host}:${port}${route}`));
