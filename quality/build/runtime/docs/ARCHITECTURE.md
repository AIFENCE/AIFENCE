...
```text
AIFENCE Core 1.8.8 (immutable, vendored, hash-locked)
          │
    CoreProvider
          │
...
`core/` is an exact Revision 1.8.8 vendoring and `CORE_LOCK.json` hashes every file. Runtime changes do not rewrite Core. A future Core upgrade is a deliberate re-vendor operation followed by tests and a new Runtime release.
