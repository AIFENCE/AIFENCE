...
The complete AIFENCE control plane. `source/README.md` is authoritative. This tree owns all semantic truth: routing, profiles, stable control IDs, artifact contracts, operations standards, schemas, evals and validators.

Files in `source/` may be consumed directly by an agent even if Runtime is unavailable.

## `tooling/` — build implementation

Contains the deterministic build/release scripts plus templates/runtime implementation used to generate cross-model distributions. Changes here affect packaging/interoperability, not AIFENCE policy unless accompanied by a canonical source change.
